...
def parse_clargs():
    ...
    P.add_argument('-e', '--edir', type=str, default='.',
                   help='the path with environment scripts for each simulator.')
...
for ncells in cell_range:
    ...
    nrn_run_fid.write('nrn_ofile="$odir/nrn_%s".out\n'%(run_name))
    nrn_run_fid.write('run_with_mpi $ns_python "%s/neuron/run.py" --mpi --param %s --opath "%s" --ipath "%s" --dump > "$nrn_ofile"\n'%(bdir, fname, odir, idir))
    ...
    cnr_run_fid.write('corenrn_ofile="$odir/corenrn_%s".out\n'%(run_name))
    cnrn_input_path=('%s/%s_core'%(idir, run_name))
    cnr_run_fid.write('if [ -d "%s" ]; then\n'%(cnrn_input_path))
    cnr_run_fid.write('  run_with_mpi coreneuron_exec -mpi -d "%s" -e %s --outpath "%s" &> "$corenrn_ofile"\n'%(cnrn_input_path, str(duration), odir))
    cnr_run_fid.write('  coreneuron_table_line "$corenrn_ofile"\n')
    cnr_run_fid.write('  [ -f "$odir/out.dat" ] && mv "$odir/out.dat" "$odir/corenrn_%s_spikes.dat"\n'%(run_name))
    ...
    arb_run_fid.write('arb_ofile="$odir/arb_%s".out\n'%(run_name))
    # TODO: move executable to binary path?
    arb_run_fid.write('run_with_mpi "%s/arbor/run" "%s" > $arb_ofile\n'%(bdir, fname))
